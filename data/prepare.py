...
def prepare_dataset(args):
    ...
    assert (path/"Gu2020_datashare\datashare.parquet").exists(), (
        "'Gu2020_datashare/datashare.parquet' does not exist in data path. "
        "If only .csv file is available, convert first using the helper "
        "function 'csv_to_parquet'.")
    ...
    # 7 lines (line 28278-28284) in the .csv. were added manually, after 
    ...
    # Filter opt_df to only contain dates that are withing 'sdate' and 'edate'.
    ...
    if fill_na:
        ...
        print(f"Fill missing values... There are {gu2020_df.isnull().to_numpy().sum()} "
                f"missing values in the Gu2020 dataset. Fill them with {fill_fn} of stocks of "
                "respective month...")
    ...
    save_df(path, final_df, size=size, fill_na=fill_na, fill_fn=fill_fn)
    print("Done!")
    print("---")
    print("All done.")


def save_df(path: Path, final_df: pd.DataFrame, size: str, fill_na: bool, fill_fn: str):
    ...
    if size == "small":
        ...
        if fill_na:  # NaNs filled.
            filename = prefix + f"_med_fill{fill_fn}.parquet"
        ...
        if fill_na: # NaNs filled.
            filename= prefix + f"_big_fill{fill_fn}.parquet"
        else: # NaNs not filled
...
# Run the function if this file is run.
if __name__ == "__main__":

    parser = ArgumentParser(description="Loads 'sp500_opt_ret.parquet', 'mapping_table.csv'" 
    "and 'Gu2020_datashare\datashare.parquet' files. Merges them according to the "
    ...
    # 'Gu2020_datashare\datashare.parquet' should reside.
